...
def combination_display_task(
    ...
    # 상의 이미지 로드
    for top_file in sorted(tops_dir.glob('T*.jpg')):
        top_images.append(pygame.image.load(str(top_file)))
    
    # 하의 이미지 로드
    for bottom_file in sorted(bottoms_dir.glob('B*.jpg')):
        bottom_images.append(pygame.image.load(str(bottom_file)))

    # CSV 파일 생성 및 헤더 작성
    with open(filename, mode="w", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(["ISI", "RT", "Response", "Top_Number", "Bottom_Number"])
        ...
        for top_idx, top_image in enumerate(top_images):
            for bottom_idx, bottom_image in enumerate(bottom_images):
                ...
                # 결과를 CSV에 기록
                writer.writerow([isi, rt, response, top_idx + 1, bottom_idx + 1])
